...
class MainDialogImgBW(QMainWindow, Ui_form):
    ...
    def init_F(self):
        set_left = pg.AxisItem('left', pen='y', textPen='y', units='uV', maxTickLength=-5)
        set_bottom = pg.AxisItem('bottom', pen='y', textPen='y', maxTickLength=-5)
    ...
    def plotData(self):
        ...
        try:
            ...
            x_ = self.N / 2 * np.linspace(0, 1, self.N / 2)
            transformed = 1.0 / self.N * np.abs(np.fft.fft(x, self.N))
            self.curve5.setData(x_, transformed[0:self.N / 2], )
